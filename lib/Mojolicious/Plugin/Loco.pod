...
Which browser (executable file) to run.  If false (C<0> or C<''>) then no browser will be invoked.  If a subroutine reference, then call it; first argument will be the initial URI.
...
Performs the C<csrf_token> validation; returns true and renders an HTTP 400 reply if it fails, otherwise returns false.  If L</allow_other_browsers> is false, this also calls L</loco.id_fail> implicitly.
...
Using a listening point that is network accessible means the server will (at least initially) accept incoming connection attempts from untrusted remote sites unless you have a reliable firewall to block these.  If, in addition, L</allow_other_browsers> is also set, then these remote clients will be able to obtain working sessions.
