...
How many seconds for the server to wait for the initial browser window to finish loading before giving up and shutting down.  Zero means wait forever.  Default is 15.  Settable from L</loco-E<gt>conf> as long as you do it before server start.

=head2 final_wait

How many seconds for the server to wait after the last browser window ceases communicating before shutting down (we do not rely on C<window.unload>).  Default is 3.  Settable from L</loco-E<gt>conf>.
